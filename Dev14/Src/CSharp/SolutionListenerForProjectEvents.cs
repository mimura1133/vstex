...
namespace VsTeXProject.VisualStudio.Project
{
    ...
    internal class SolutionListenerForProjectEvents : SolutionListener, IProjectEvents
    {
        ...
        {
            var projectEventListener = hierarchy as IProjectEventsListener;
            if (projectEventListener != null && projectEventListener.IsProjectEventsListener)
            {
                RaiseAfterProjectFileOpened(added != 0 ? true : false);
            }
        ...
        {
            var projectEvents = hierarchy as IProjectEventsListener;
            if (projectEvents != null && projectEvents.IsProjectEventsListener)
            {
                RaiseBeforeProjectFileClosed(removed != 0 ? true : false);
            }
        ...
        /// </summary>
        /// <param name="added">
        ///     True if the project is added to the solution after the solution is opened. false if the project is
        ///     added to the solution while the solution is being opened.
        /// </param>
        private void RaiseAfterProjectFileOpened(bool added)
        {
            // Save event in temporary variable to avoid race condition.
            var tempEvent = AfterProjectFileOpened;
            if (tempEvent != null)
            {
        ...
        /// </summary>
        /// <param name="added">
        ///     true if the project was removed from the solution before the solution was closed. false if the
        ///     project was removed from the solution while the solution was being closed.
        /// </param>
        private void RaiseBeforeProjectFileClosed(bool removed)
        {
            // Save event in temporary variable to avoid race condition.
            var tempEvent = BeforeProjectFileClosed;
            if (tempEvent != null)
            {
