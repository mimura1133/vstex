...
namespace VsTeXProject.VisualStudio.Project
{
    ...
    public class NestedProjectNode : HierarchyNode, IPropertyNotifySink
    {
        ...
        internal IVsHierarchy NestedHierarchy { get; private set; }
        ...
        ///     The client (owner of the sink) should then retrieve the current value of each property of interest from the object
        ///     that generated the notification.
        ///     In our case we will care about the  VSLangProj80.VsProjPropId.VBPROJPROPID_FileName and update the changes in the
        ///     parent project file.
        /// </summary>
        /// <param name="dispid">
        ///     Dispatch identifier of the property that is about to change or DISPID_UNKNOWN if multiple
        ...
        public virtual void OnChanged(int dispid)
        {
            if (dispid == (int) VsProjPropId.VBPROJPROPID_FileName)
            {
                ...
                var nestedProject = NestedHierarchy as IVsProject3;
        ...
        /// <summary>
        ///     Notifies a sink that a [requestedit] property is about to change and that the object is asking the sink how to
        ///     proceed.
        /// </summary>
        /// <param name="dispid">
        ///     Dispatch identifier of the property that is about to change or DISPID_UNKNOWN if multiple
        ...
        {
            var vshPropId = (__VSHPROPID) propId;
            switch (vshPropId)
            {
                default:
        ...
        /// <summary>
        ///     Gets the icon handle. It tries first the nested to get the icon handle. If that is not supported it will get it
        ///     from
        ...
        public override object GetIconHandle(bool open)
        {
            ...
            NestedHierarchy.GetProperty(VSConstants.VSITEMID_ROOT, (int) __VSHPROPID.VSHPROPID_IconHandle,
                out iconHandle);
            if (iconHandle == null)
            {
                ...
                if (imageHandler.ImageList != null)
                {
                    ...
                    {
                        var imageIndex = (int) imageIndexAsObject;
        ...
        /// <summary>
        ///     Return S_OK. Implementation of Closing a nested project is done in CloseNestedProject which is called by
        ///     CloseChildren.
        /// </summary>
        ...
        {
            var result = DelegateSetPropertyToNested((int) __VSHPROPID.VSHPROPID_EditLabel, label);
        ...
        protected internal override void ReloadItem(uint reserved)
        {
            ...
            var persistHierachyItem = NestedHierarchy as IVsPersistHierarchyItem2;
            ...
            __VSCREATEPROJFLAGS createFlags)
        {
            ...
            var typeGuidString = ItemNode.GetMetadataAndThrow(ProjectFileConstants.TypeGuid,
                new InvalidOperationException());
            var projectFactoryGuid = Guid.Empty;
            ...
            int cancelled;
            var refiid = NativeMethods.IID_IUnknown;
            var projectPtr = IntPtr.Zero;

            try
            {
                ErrorHandler.ThrowOnFailure(projectFactory.CreateProject(fileName, destination, projectName,
                    (uint) createFlags, ref refiid, out projectPtr, out cancelled));
            ...
            ErrorHandler.ThrowOnFailure(NestedHierarchy.SetProperty(VSConstants.VSITEMID_ROOT,
                (int) __VSHPROPID.VSHPROPID_ParentHierarchy, ProjectMgr));
            ErrorHandler.ThrowOnFailure(NestedHierarchy.SetProperty(VSConstants.VSITEMID_ROOT,
                (int) __VSHPROPID.VSHPROPID_ParentHierarchyItemid, (int) ID));
        ...
        protected virtual void LockRDTEntry()
        {
            ...
            var flags = _VSRDTFLAGS.RDT_VirtualDocument | _VSRDTFLAGS.RDT_ProjSlnDocument;
            ;
            ...
            try
            {
                ...
                else
                {
                    ...
                    ErrorHandler.ThrowOnFailure(rdt.RegisterAndLockDocument((uint) flags, projectPath,
                        ProjectMgr.InteropSafeIVsHierarchy, ID, projectPtr, out docCookie));
        ...
        protected virtual void UnlockRDTEntry()
        {
            ...
            {
                var flags = _VSRDTFLAGS.RDT_EditLock;
        ...
        protected virtual void RenameNestedProjectInParentProject(string label)
        {
            ...
            try
            {
                ...
                var newFileName = label + Path.GetExtension(oldFileName);
                SaveNestedProjectItemInProjectFile(newFileName);

                var projectDirectory = Path.GetDirectoryName(oldFileName);
        ...
        {
            var existingInclude = ItemNode.Item.EvaluatedInclude;
            var existingRelativePath = Path.GetDirectoryName(existingInclude);
            var newRelativePath = Path.Combine(existingRelativePath, newFileName);
        ...
        private void InitializeInstanceGuid()
        {
            ...
            if (ProjectMgr.IsNewProject)
            {
                ...
                ErrorHandler.ThrowOnFailure(NestedHierarchy.SetGuidProperty(VSConstants.VSITEMID_ROOT,
                    (int) __VSHPROPID.VSHPROPID_ProjectIDGuid, ref instanceGuid));
            }
            else
            {
                ...
                ErrorHandler.ThrowOnFailure(NestedHierarchy.GetGuidProperty(VSConstants.VSITEMID_ROOT,
                    (int) __VSHPROPID.VSHPROPID_ProjectIDGuid, out nestedHiererachyInstanceGuid));

                // Get instance guid from the project file. If it does not exist then we create one.
                var instanceGuidAsString = ItemNode.GetMetadata(ProjectFileConstants.InstanceGuid);
        ...
        private void SetDocCookieOnNestedHier(uint itemDocCookie)
        {
            ...
            {
                ErrorHandler.ThrowOnFailure(NestedHierarchy.SetProperty(VSConstants.VSITEMID_ROOT,
                    (int) __VSHPROPID.VSHPROPID_ItemDocCookie, docCookie));
            }
        ...
        private void InitImageHandler()
        {
            ...
            ErrorHandler.ThrowOnFailure(NestedHierarchy.GetProperty(VSConstants.VSITEMID_ROOT,
                (int) __VSHPROPID.VSHPROPID_IconImgList, out imageListAsPointer));
        ...
        private object DelegateGetPropertyToNested(int propID)
        {
            if (!ProjectMgr.IsClosed)
            {
                ...
                var result = NestedHierarchy.GetProperty(VSConstants.VSITEMID_ROOT, propID, out returnValue);
                if (ErrorHandler.Succeeded(result))
                {
                    return returnValue;
                }
        ...
        {
            var parent = ProjectMgr as ProjectContainerNode;
        ...
        private void ConnectPropertyNotifySink()
        {
            ...
            var connectionPoint = GetConnectionPointFromPropertySink();
        ...
        private IConnectionPoint GetConnectionPointFromPropertySink()
        {
            ...
            var connectionPointContainer = browseObject as IConnectionPointContainer;

            if (connectionPointContainer != null)
            {
                var guid = typeof (IPropertyNotifySink).GUID;
