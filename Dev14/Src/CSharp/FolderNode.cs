...
namespace VsTeXProject.VisualStudio.Project
{
    ...
    public class FolderNode : HierarchyNode
    {
        ...
        {
            return
                ProjectMgr.ImageHandler.GetIconHandle(open
                    ? (int) ProjectNode.ImageName.OpenFolder
                    : (int) ProjectNode.ImageName.Folder);
        }
        ...
        {
            if (string.Compare(Path.GetFileName(Url.TrimEnd('\\')), label, StringComparison.Ordinal) == 0)
            {
                // Label matches current Name
                return VSConstants.S_OK;
            }

            var newPath = Path.Combine(new DirectoryInfo(Url).Parent.FullName, label);
            ...
            try
            {
                ...
                var shell = ProjectMgr.GetService(typeof (SVsUIShell)) as IVsUIShell;
            ...
            catch (Exception e)
            {
                throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture,
                    SR.GetString(SR.RenameFolder, CultureInfo.CurrentUICulture), e.Message));
            }
            return VSConstants.S_OK;
        }
        ...
        {
            get { return VSConstants.GUID_ItemType_PhysicalFolder; }
        }

        public override string Url
        {
            get { return Path.Combine(Path.GetDirectoryName(ProjectMgr.Url), VirtualNodeName) + "\\"; }
        }
            ...
            IList<tagVsSccFilesFlags> flags)
        {
            ...
            var node = FindChild(sccFile) as FileNode;
        ...
        /// </summary>
        [SuppressMessage("Microsoft.Performance", "CA1804:RemoveUnusedLocals", MessageId = "e")]
        ...
        public virtual void CreateDirectory(string newName)
        {
            ...
            try
            {
                ...
                var strNewDir = Path.Combine(Path.GetDirectoryName(oldDir), newName);
        ...
        private void RenameFolder(string newName)
        {
            ...
            var newPath = Path.Combine(Parent.VirtualNodeName, newName);
        ...
        private int ShowFileOrFolderAlreadExistsErrorMessage(string newPath)
        {
            ...
            var errorMessage = string.Format(CultureInfo.CurrentCulture,
                SR.GetString(SR.FileOrFolderAlreadyExists, CultureInfo.CurrentUICulture), newPath);
            if (!Utilities.IsInAutomationFunction(ProjectMgr.Site))
            {
                ...
                VsShellUtilities.ShowMessageBox(ProjectMgr.Site, title, errorMessage, icon, buttons, defaultButton);
                return VSConstants.S_OK;
            }
