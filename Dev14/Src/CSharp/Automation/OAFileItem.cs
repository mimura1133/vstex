...
namespace VsTeXProject.VisualStudio.Project.Automation
{
    ...
    public class OAFileItem : OAProjectItem<FileNode>
    {
                ...
                using (var scope = new AutomationScope(Node.ProjectMgr.Site))
                {
                    ...
                    try
                    {
                        ...
                        ErrorHandler.ThrowOnFailure(rdt.FindAndLockDocument((uint) _VSRDTFLAGS.RDT_NoLock, url,
                            out ivsHierarchy, out itemid, out docData, out docCookie));
                        ...
                        else
                        {
                            ...
                            if (!isCalledFromSaveAs)
                            {
                                ...
                                ErrorHandler.ThrowOnFailure(Node.ProjectMgr.SaveItem(VSSAVEFLAGS.VSSAVE_SilentSave,
                                    fullPath, Node.ID, docData, out canceled));
                            }
                            else
                            {
                                ErrorHandler.ThrowOnFailure(Node.ProjectMgr.SaveItem(VSSAVEFLAGS.VSSAVE_SilentSave,
        ...
        ///     Is thrown if the project is closed or it the service provider attached to
