...
namespace VsTeXProject.VisualStudio.Project
{
    ...
    internal class FileChangeManager : IVsFileChangeEvents
    {
        ...
        public void Dispose()
        {
            ...
            foreach (var info in observedItems.Values)
            {
        ...
        /// <summary>
        ///     Maps between the observed item identified by its filename (in canonicalized form) and the cookie used for
        ///     subscribing
        ///     to the events.
        /// </summary>
        ...
        int IVsFileChangeEvents.FilesChanged(uint numberOfFilesChanged, string[] filesChanged, uint[] flags)
        {
            ...
            if (FileChangedOnDisk != null)
            {
                for (var i = 0; i < numberOfFilesChanged; i++)
                {
                    ...
                    {
                        var info = observedItems[fullFileName];
                        FileChangedOnDisk(this,
                            new FileChangedOnDiskEventArgs(fullFileName, info.ItemID, (_VSFILECHANGEFLAGS) flags[i]));
                    }
                }
            }
        ...
        /// <summary>
        ///     Observe when the given file is updated on disk. In this case we do not care about the item id that represents the
        ...
        internal void ObserveItem(string fileName, uint id)
        {
            ...
            if (!observedItems.ContainsKey(fullFileName))
            {
                ...
                ErrorHandler.ThrowOnFailure(fileChangeService.AdviseFileChange(fullFileName,
                    (uint) (_VSFILECHANGEFLAGS.VSFILECHG_Time | _VSFILECHANGEFLAGS.VSFILECHG_Del), this,
        ...
        internal void StopObservingItem(string fileName)
        {
            ...
            if (observedItems.ContainsKey(fullFileName))
            {
                ...
                var itemInfo = observedItems[fullFileName];
