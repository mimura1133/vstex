...
namespace VsTeXProject.VisualStudio.Project
{
    ...
    public sealed class ProjectElement
    {
        ...
        private readonly Dictionary<string, string> virtualProperties;
        ...
        internal MSBuild.ProjectItem Item { get; private set; }
        ...
        {
            Debug.Assert(
                string.Compare(attributeName, ProjectFileConstants.Include, StringComparison.OrdinalIgnoreCase) != 0,
                "Use rename as this won't work");
        ...
        public string GetMetadataAndThrow(string attributeName, Exception exception)
        {
            Debug.Assert(!string.IsNullOrEmpty(attributeName),
                "Cannot retrieve an attribute for a null or empty attribute name");
            var attribute = GetMetadata(attributeName);

            if (string.IsNullOrEmpty(attributeName) && exception != null)
            {
                if (string.IsNullOrEmpty(exception.Message))
                {
                    Debug.Assert(!string.IsNullOrEmpty(itemProject.BaseURI.AbsoluteUrl),
                        "Cannot retrieve an attribute for a project that does not have a name");
                    var message = string.Format(CultureInfo.CurrentCulture,
                        SR.GetString(SR.AttributeLoad, CultureInfo.CurrentUICulture), attributeName,
                        itemProject.BaseURI.AbsoluteUrl);
        ...
        {
            var escapedPath = MSBuild.ProjectCollection.Escape(newPath);
            if (IsVirtual)
            {
        ...
        public void RefreshProperties()
        {
            ...
            IEnumerable<MSBuild.ProjectItem> items = itemProject.BuildProject.GetItems(Item.ItemType);
        ...
        {
            var path = GetMetadata(ProjectFileConstants.Include);
            if (!Path.IsPathRooted(path))
                path = Path.Combine(itemProject.ProjectFolder, path);
        ...
        public static bool operator ==(ProjectElement element1, ProjectElement element2)
        {
            ...
            var include1 = element1.GetMetadata(ProjectFileConstants.Include);
            var include2 = element2.GetMetadata(ProjectFileConstants.Include);
        ...
        {
            var element2 = obj as ProjectElement;
            if (element2 == null)
                return false;
