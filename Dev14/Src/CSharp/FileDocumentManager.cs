...
namespace VsTeXProject.VisualStudio.Project
{
    ...
    public class FileDocumentManager : DocumentManager
    {
        ...
        public virtual int Open(bool newFile, bool openWith, ref Guid logicalView, IntPtr docDataExisting,
            out IVsWindowFrame windowFrame, WindowFrameShowAction windowFrameAction)
        {
            windowFrame = null;
            var editorType = Guid.Empty;
            return Open(newFile, openWith, 0, ref editorType, null, ref logicalView, docDataExisting, out windowFrame,
                windowFrameAction);
        }
        ...
        private int Open(bool newFile, bool openWith, uint editorFlags, ref Guid editorType, string physicalView,
            ref Guid logicalView, IntPtr docDataExisting, out IVsWindowFrame windowFrame,
            WindowFrameShowAction windowFrameAction)
        {
            ...
            var caption = GetOwnerCaption();
            var fullPath = GetFullPathForDocument();
            ...
            try
            {
                ...
                if (openWith)
                {
                    result = uiShellOpenDocument.OpenStandardEditor((uint) __VSOSEFLAGS.OSE_UseOpenWithDialog, fullPath,
                        ref logicalView, caption, Node.ProjectMgr.InteropSafeIVsUIHierarchy, Node.ID, docDataExisting,
                        serviceProvider, out windowFrame);
                }
                else
                {
                    ...
                    {
                        result = uiShellOpenDocument.OpenSpecificEditor(editorFlags, fullPath, ref editorType,
                            physicalView, ref logicalView, caption, Node.ProjectMgr.InteropSafeIVsUIHierarchy, Node.ID,
                            docDataExisting, serviceProvider, out windowFrame);
                    }
                    else
                    {
                        openFlags |= __VSOSEFLAGS.OSE_ChooseBestStdEditor;
                        result = uiShellOpenDocument.OpenStandardEditor((uint) openFlags, fullPath, ref logicalView,
                            caption, Node.ProjectMgr.InteropSafeIVsUIHierarchy, Node.ID, docDataExisting,
                            serviceProvider, out windowFrame);
                    }
                }

                if (result != VSConstants.S_OK && result != VSConstants.S_FALSE &&
            ...
            WindowFrameShowAction windowFrameAction)
        {
            var newFile = false;
            var openWith = false;
            return Open(newFile, openWith, ref logicalView, docDataExisting, out windowFrame, windowFrameAction);
        }
            ...
            WindowFrameShowAction windowFrameAction)
        {
            ...
            var newFile = false;
            var openWith = false;
            return Open(newFile, openWith, editorFlags, ref editorType, physicalView, ref logicalView, docDataExisting,
        ...
        public int Open(bool newFile, bool openWith, WindowFrameShowAction windowFrameAction)
        {
            ...
            return Open(newFile, openWith, logicalView, out windowFrame, windowFrameAction);
        }
        ...
        /// <param name="windowFrameAction">Determine the UI action on the document window</param>
        /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code.</returns>
        public int Open(bool newFile, bool openWith, Guid logicalView, out IVsWindowFrame frame,
            WindowFrameShowAction windowFrameAction)
        {
            frame = null;
            ...
            try
            {
                ErrorHandler.ThrowOnFailure(rdt.FindAndLockDocument((uint) flags, path, out ivsHierarchy, out itemid,
                    out docData, out docCookie));
                ErrorHandler.ThrowOnFailure(Open(newFile, openWith, ref logicalView, docData, out frame,
                    windowFrameAction));
            }
            catch (COMException e)
            {
